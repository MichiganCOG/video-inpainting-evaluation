...
def _ssim(a, b):
    """Convenience wrapper to compute SSIM between RGB images a and b."""
    return structural_similarity(a, b, multichannel=True)


class PConsSSIMComputer(MetricComputer):

    def compute_metric(self):
        self.send_work_count_msg(len(self.opts.video_names))
    
        pcons_ssim_all = np.zeros(len(self.opts.video_names))
                ...
                for b_sy in range(a_sy - self.opts.sim_cons_sw, a_sy + self.opts.sim_cons_sw):
                    for b_sx in range(a_sx - self.opts.sim_cons_sw, a_sx + self.opts.sim_cons_sw):
                        ...
                        if comp_frame_a_patch.shape != comp_frame_b_patch.shape:
                            ...
                            continue
                        patch_ssim = _ssim(comp_frame_a_patch, comp_frame_b_patch)
                        if patch_ssim > best_patch_ssim:
                            best_patch_ssim = patch_ssim
                            best_b_sy = b_sy
                            best_b_sx = b_sx
    
                best_comp_frame_b_patch = comp_frame_b[best_b_sy:best_b_sy + self.opts.sim_cons_ps,
                                                       best_b_sx:best_b_sx + self.opts.sim_cons_ps]
                pcons_ssim_all[v] += _ssim(comp_frame_a_patch, best_comp_frame_b_patch) / (num_frames - 1)
    
                comp_frame_a = comp_frame_b
    
            self.send_update_msg(1)
    
        self.send_result_msg(pcons_ssim_all)
